...
# Ground truth visual targets in 1920x1080:
# Frame 1:
#   up-fin (Left): (997.29, 461.10)
#   down-fin (Right): (1004.01, 551.40)
# Frame 3:
#   up-fin (Left): (970.0, 495.0)
#   down-fin (Right): (975.0, 550.0)
...
def optimize_constrained(func, start_val, args=(), steps=[10.0, 5.0, 1.0, 0.1, 0.01, 0.001]):
        ...
        while improved:
            ...
            for i in range(len(best_params)):
                for delta in [-step, step]:
                    ...
                    # y in [10, 30]
                    # z in [10, 30]
                    if i == 0 and not (-45.0 <= test_params[0] <= -30.0):
                        continue
                    if i == 1 and not (10.0 <= test_params[1] <= 30.0):
                        continue
                    if i == 2 and not (10.0 <= test_params[2] <= 30.0):
                        continue
...
# Frame 1: Left=(997.29, 461.10), Right=(1004.01, 551.40)
# Frame 3: Left=(970.0, 495.0), Right=(975.0, 550.0)
print("Running constrained optimizer...")
for x in linspace(-45, -30, 6):
    for y in linspace(10, 30, 6):
        for z in linspace(10, 30, 6):
            p, loss = optimize_constrained(loss_symmetric_pair, [x, y, z], 
                                          args=((997.29, 461.10), (1004.01, 551.40), (970.0, 495.0), (975.0, 550.0)))
            if loss < best_loss:
                best_loss = loss
                best_p = p

print(f"Optimal constrained coords: x={best_p[0]:.4f}, y={best_p[1]:.4f}, z={best_p[2]:.4f}")
print(f"Total loss: {best_loss:.4f} (RMSE = {math.sqrt(best_loss/4):.4f} pixels)")
