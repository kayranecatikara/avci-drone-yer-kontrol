...
def main():
    ...
    if not os.path.exists(annotated_dir):
        ...
        print(f"[INFO] Created annotated directory: {annotated_dir}")
            ...
            if content.startswith("{") and content.endswith("}"):
                ...
                if data.get("status") == "MANUAL_SHOT":
                    ...
                    if rect[2] > rect[0] and rect[3] > rect[1]:
                        ...
                        # Temiz hali zaten dataset'e kaydedildi; noktali kopya
                        # dataset_annotated'a gider (temizin uzerine YAZILMAZ)
                        if metadata["keypoints_2d"]:
                            ...
                            annotated.save(os.path.join(annotated_dir, f"manual_shot_{ts}.png"), "PNG")
                    ...
                    if index != last_processed:
                        ...
                        if right > left and bottom > top:
                            ...
                            # TEMIZ fotograf datasete kaydedilir (nokta YOK)
                            img.save(save_path, "PNG")

                            # Noktali kopya ayrica dataset_annotated'a kaydedilir
                            if metadata["keypoints_2d"]:
                                annotated = img.copy()
                                draw = ImageDraw.Draw(annotated)
                                for name, pt in metadata["keypoints_2d"].items():
                                    if pt.get("on", False):
                                        ...
                                        r = 2 # yaricap
                                        draw.ellipse((px-r, py-r, px+r, py+r), fill="red", outline="white")
                                annotated.save(os.path.join(annotated_dir, filename), "PNG")
