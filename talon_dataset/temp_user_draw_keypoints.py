...
# ============================================================================
# PIVOT -> CG DUZELTMESI  (UE GOVDE EKSENINDE, SANTIMETRE)
# ----------------------------------------------------------------------------
# CAD noktalari AGIRLIK MERKEZINE (origin = 0,0,0) gore tanimli.
# Ama UE'nin K2_GetActorLocation() verdigi "drone_location" actor PIVOT'udur.
# Eger pivot ile CG ayni noktaysa bunu (0,0,0) birak.
# Eger DEGILSE: kamera HER ZAMAN pivot'a bakar, yani ekran ORTASI (960,540)
# modeldeki pivot noktasini gosterir. Ekran ortasi neyi gosteriyorsa pivot orada.
#   - Ekran ortasi BURUN ucundaysa  -> pivot burunda  -> {"x": -55.43, "y": 0.0, "z": 1.27}
#   - Ekran ortasi MOTOR/kuyruk ucundaysa -> pivot kuyrukta -> {"x": 56.02, "y": 0.0, "z": 4.38}
#   - Ekran ortasi tam KANAT KOKUNDEYSE -> pivot = CG -> {"x":0,"y":0,"z":0}  (degisiklik yok)
# (Bu offset = CG_govde(0,0,0) - pivot_govde, yani -1 * pivotun govde koordinati.)
# UE govde ekseni: +X = ileri (burun), +Y = sag, +Z = yukari.  cm cinsinden.
PIVOT_TO_CG_OFFSET_CM = {"x": 0.0, "y": 0.0, "z": 0.0}

# Ekran merkezine arti cizilsin mi? (pivot her zaman buraya duser -> CG kontrolu)
DRAW_CENTER_CROSSHAIR = True
...
def rotate_vector_ue(x, y, z, pitch, yaw, roll):
    ...
    # AxisZ (Up)  -- FIX: önceki hali UE'nin gerçek Up vektörü değildi
    # (X ve Y dünya bileşenleri ters işaretliydi). Doğru UE Up = Forward x Right.
    r02 = -(CR * SP * CY + SR * SY)
    r12 = SR * CY - CR * SP * SY
    r22 = CR * CP
...
def calculate_keypoints_2d(drone_loc, drone_rot, cam_loc, cam_rot, cam_fov=90.0):
    ...
    for name, cad in RAW_CAD_DATA.items():
        ...
        # FIX: CAD sag-elli (+Z = SOL), UE sol-elli (+Y = SAG). El sistemi
        # degistigi icin bu ekseni TERS cevirmek zorundayiz -> ue_y = -cad_z
        ue_x = -cad["x"] / 10.0
        ue_y = -cad["z"] / 10.0
        ue_z = cad["y"] / 10.0
        
        # Pivot != CG ise govde ekseninde sabit offset uygula (bkz. ust taraf)
        ue_x += PIVOT_TO_CG_OFFSET_CM["x"]
        ue_y += PIVOT_TO_CG_OFFSET_CM["y"]
        ue_z += PIVOT_TO_CG_OFFSET_CM["z"]
...
def main():
    ...
    for filename in sorted(files):
        ...
        try:
            ...
            # EKRAN MERKEZI ARTISI: kamera pivot'a baktigi icin pivot her zaman
            # tam buraya duser. Bu artinin altinda Talon'un AGIRLIK MERKEZI
            # (kanat koku) gorunmuyorsa -> pivot != CG -> PIVOT_TO_CG_OFFSET_CM ayarla.
            if DRAW_CENTER_CROSSHAIR:
                cxp, cyp = 960, 540
                draw_ann.line([cxp - 22, cyp, cxp + 22, cyp], fill=(0, 255, 200), width=2)
                draw_ann.line([cxp, cyp - 22, cxp, cyp + 22], fill=(0, 255, 200), width=2)
                draw_ann.ellipse([cxp - 26, cyp - 26, cxp + 26, cyp + 26], outline=(0, 255, 200), width=2)
