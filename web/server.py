...
def dedektor_dongusu():
    ...
    while True:
        ...
        try:
            ...
            # ACAMAZ ama mevcut izi yasatir (zayif karede ID kopmaz).
        ...
        if bgr is not None:
            ...
            try:
                ...
                if onceki_att is not None:
                    H_cmc = kamera_model.cmc_homografi(bgr.shape[1], bgr.shape[0],
                                                       onceki_att, att)
            except Exception:
                att = None
            dt_takip = (simdi - onceki_takip_t) if onceki_takip_t is not None else 0.05
            onceki_att, onceki_takip_t = att, simdi
            det = takipci.guncelle(dets, dt_takip, H_cmc)
            if det is not None:
