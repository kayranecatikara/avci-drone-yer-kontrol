...
local KEYPOINTS_LOCAL = {
    Nose = {X = 61.11, Y = -0.07, Z = -2.32},
    Left_Wingtip = {X = 3.50, Y = -89.00, Z = 4.66},
    Right_Wingtip = {X = 1.50, Y = 88.91, Z = 5.09},
    Tail = {X = -48.81, Y = 0.03, Z = 0.56},
    Left_Tail_Fin = {X = -38.86, Y = 24.80, Z = 15.16},
    Right_Tail_Fin = {X = -37.80, Y = -25.02, Z = 15.61}
}
...
local function GenerateDiverseCombo(captureIndex)
    ...
    for attempt = 1, 100 do
        ...
        combo = {
            -- Kullanici istegi uzerine mesafe 2m (200cm) ile 12m (1200cm) arasi
            dist = math.random(200, 1200),
        ...
        -- "SIFIR BENZERLIK - MAKSIMUM YAPAY ZEKA EGITIMI" ISTEGI:
        -- Modelin her turlu durumu ogrenmesi icin "Mantikli Kovalamaca" kuralini (önceki istek) cöpe attik.
        -- Drone'a %100 her taraftan (Kafa kafaya, arkadan, yandan, caprazdan) bakilacak.
        local targetLocalYaw = math.random(0, 359)
        combo.cyaw = (combo.dyaw + targetLocalYaw) % 360
        
        -- Dikey acidan (YUKSEKLIK) hicbir kisitlama yok (Tam tepe, tam burun alti her yer dahil)
        combo.cpitch = math.random(-85, 85)
        
        -- KULLANICI ISTEGI: "Ters donmesin, düzgün baksin"
        -- Drone artik tepetaklak (upside down) OLMAYACAK. Sadece normal ucarkenki gibi hafif saga sola yatacak.
        combo.rollTilt = math.random(-25, 25)
        
        local tooSimilar = false
          local startIdx = math.max(1, #poseHistory - 100) -- Sadece son 100 fotografa bak! 12 bin fotoya bakarsa her seyi reddeder.
