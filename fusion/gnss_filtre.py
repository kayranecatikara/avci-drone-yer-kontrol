...
class GNSSFiltre:
    ...
    def guncelle(self, bozuk_x, bozuk_y, bozuk_z):
        ...
        if dt_son <= 1e-3:
            dt_son = 0.2
        vadx = (xt[-1] - xt[-2]) / dt_son * M_TO_CM
        vady = (yt[-1] - yt[-2]) / dt_son * M_TO_CM
        vadz = (zt[-1] - zt[-2]) / dt_son * M_TO_CM
        fark = ((vadx - self._vlead[0]) ** 2 + (vady - self._vlead[1]) ** 2
                + (vadz - self._vlead[2]) ** 2) ** 0.5
