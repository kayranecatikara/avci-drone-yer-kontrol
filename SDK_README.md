...
- `drone_gui.py`: Test ve manuel kontrol için geliştirilmiş modern arayüz. (Otonom modu içerir)
- `contestant_example_v2.py`: Temel bir AI şablonu.
...
- Hedefi görüntü merkezine almak veya nişan hesaplamak için kamera tilt açısını (25°) pitch hesaplarında telafi etmeniz gerekebilir (`contestant_example_v2.py` içindeki `CAMERA_TILT` sabitine bakın).
...
- **Düşük güncelleme hızı (hedef GPS):** Hedef (Talon) konum/hız telemetrisi gerçek bir GPS gibi **en fazla saniyede 1 kez (1 Hz)** güncellenir. Kendi dronunuzun telemetrisi tam hızda ve temiz gelir.
...
- **Sinyal kesintisi (dropout):** Veri kısa süreliğine tamamen kesilebilir. Bu sırada SDK bağlantıyı
  **koparmaz**; `get_` fonksiyonları son bilinen değeri döndürmeye devam eder.
